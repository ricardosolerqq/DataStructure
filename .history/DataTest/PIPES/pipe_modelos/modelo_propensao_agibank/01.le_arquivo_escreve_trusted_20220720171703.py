...
if escreverVariavelResposta == 'True':
  df = escreve_variavel_resposta_acordo(df, 'agibank', datetime.datetime(2021,10,10), 'NR_CPF_CNPJ2', 'VARIAVEL_RESPOSTA', drop_null = True)
  ...
  dfFalse_aleatorio = dfFalse.sample((df.count()-dfTrue.count())/dfFalse.count())

  df_representativo = dfTrue.union(dfFalse_representativo)
  df_aleatorio = dfTrue.union(dfFalse_aleatorio)

  df_representativo.coalesce(1).write.option('header', 'True').option('delimiter',';').csv(os.path.join(caminho_sample, arquivo_escolhido, 'sample', arquivo_escolhido+'_amostra_representativa.csv'))
  df_aleatorio.coalesce(1).write.option('header', 'True').option('delimiter',';').csv(os.path.join(caminho_sample, arquivo_escolhido, 'sample', arquivo_escolhido+'_amostra_aleatoria.csv'))
else:
