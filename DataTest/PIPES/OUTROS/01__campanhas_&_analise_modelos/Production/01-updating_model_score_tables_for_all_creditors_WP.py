...
df = spark.read.options(header = True, delimiter = ';').csv(dir_outputs+'/arquivos_originais/Base acionada Santander_2022-03.csv')
#
...
for files in dbutils.fs.ls(dir_outputs+'/arquivos_originais/tmpWP/'):
  if files.name.split('.')[-1] == 'csv':
    dbutils.fs.cp(files.path, dir_outputs+'/arquivos_originais/'+'Base acionada Santander_2022-03_1.csv')
  else:
...
for files in dbutils.fs.ls(dir_outputs+'/arquivos_originais/tmpWP/'):
  if files.name.split('.')[-1] == 'csv':
    dbutils.fs.cp(files.path, dir_outputs+'/arquivos_originais/'+'Base acionada Santander_2022-03_2.csv')
  else:
      dbutils.fs.rm(files.path, True)
dbutils.fs.rm(os.path.join(dir_outputs, 'arquivos_originais/tmpWP'), True)         
...
display(df_aux.groupBy(F.col('data')).count())
