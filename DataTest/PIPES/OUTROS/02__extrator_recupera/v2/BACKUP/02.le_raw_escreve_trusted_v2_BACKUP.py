...
files = []
for file in os.listdir(credor.caminho_raw_dbfs):
  files.append(file)
...
dict_dfs = {}
for file in os.listdir(os.path.join(credor.caminho_raw_dbfs, arquivo_escolhido)):
  print (file)
  dict_dfs.update({file.split('.')[0]:spark.read.parquet(os.path.join(credor.caminho_raw, arquivo_escolhido,file))})
...
for df in dict_dfs_transform:
  dict_dfs_transform[df].write.mode('overwrite').parquet(os.path.join(credor.caminho_trusted, arquivo_escolhido, df+'.PARQUET'))
...
inventario_df.write.mode('overwrite').parquet(os.path.join(credor.caminho_trusted, arquivo_escolhido+'_inventario.PARQUET'))
