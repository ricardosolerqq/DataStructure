...
if processo_auto:
  try:
    ...
    pass
  arquivo_escolhido = list(get_creditor_etl_file_dates('bvm', latest=True))[0]
  ...
  arquivo_escolhido_path = os.path.join(caminho_base,dict_arq[arquivo_escolhido])
...
data_arquivo_escolhido = datetime.date(int(arquivo_escolhido[0:4]), int(arquivo_escolhido[4:6]), int(arquivo_escolhido[6:8]))
...
if escreverVariavelResposta == 'True':
  df = escreve_variavel_resposta_acordo(df, 'bvm', data_arquivo_escolhido, 'NUCPFCNPJ', drop_null = True)
