...
pre_outputpath = '/mnt/ml-prd/ml-data/propensaodeal/dmcard/output'
pre_outputpath_dbfs = '/dbfs/mnt/ml-prd/ml-data/propensaodeal/dmcard/output'
...
output = output.select('DOC', 'P_1', 'GH')

# COMMAND ----------

output = spark.read.format('csv').option('header','True').load(os.path.join(pre_outputpath, file))
output = output.drop('_c0')
output = output.withColumn('DOCUMENTO:ID_DIVIDA', F.split("DOC", ':'))
output = output.withColumn('Document', F.lpad(F.col("DOC"),11,'0'))
output = output.groupBy(F.col('Document')).agg(F.max(F.col('GH')), F.max(F.col('P_1')), F.avg(F.col('P_1')))
output = output.withColumn('Provider', F.lit('qq_dmcard_propensity_v1'))
...
for arq in dbutils.fs.ls(os.path.join(pre_outputpath, 'temp_output.csv')):
  if arq.name.split('.')[-1] == 'csv':
    print (arq.path)
    dbutils.fs.cp(arq.path, os.path.join(pre_outputpath+'/dmcard_model_to_production_'+str(createdAt)+'.csv'))
...
dbutils.fs.cp(os.path.join(pre_outputpath+'/dmcard_model_to_production_'+str(createdAt)+'.csv'), os.path.join(santander_output+'/dmcard_model_to_production_'+str(createdAt)+'.csv'))
santander_output+'/dmcard_model_to_production_'+str(createdAt)+'.csv'
