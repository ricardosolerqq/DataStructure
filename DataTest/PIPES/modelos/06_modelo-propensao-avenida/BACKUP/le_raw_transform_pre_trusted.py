...
def processo_leitura_transformacao(txtfile, data,caminho_raw, regras):
...
for date in datefile:
  for arquivo in datefile[date]:
    datas_arquivos.append(str(date)+':'+str(arquivo))

# COMMAND ----------

dbutils.widgets.combobox('DATA_EXTRACAO', max(datas_arquivos), datas_arquivos)

# COMMAND ----------

arquivo_escolhido = dbutils.widgets.get("DATA_EXTRACAO")
arquivo_escolhido = arquivo_escolhido.split(':')
data = arquivo_escolhido[0]
arquivo_escolhido = arquivo_escolhido[1]
arquivo_escolhido
...
data_arquivo = arquivo_escolhido[4:8]+'-'+arquivo_escolhido[2:4]+'-'+arquivo_escolhido[0:2]
dfs = processo_leitura_transformacao(arquivo_escolhido,data,caminho_raw, regras)
for tipo_registro in dfs:
  df = dfs[tipo_registro]
  print ('SALVANDO EM',str(parquetfilepath+'/'+data_arquivo+'/'+tipo_registro+'.PARQUET'))
  df.write.mode('overwrite').parquet(parquetfilepath+'/'+data_arquivo+'/'+tipo_registro+'.PARQUET') 
