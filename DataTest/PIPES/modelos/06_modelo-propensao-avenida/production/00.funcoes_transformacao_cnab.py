...
def leCSV(txtfile):
  ...
  df = spark.read.format("csv").option("header", False).option("encoding",'UTF-8').schema(schema_final).load(txtfile)
  return df
...
def processo_leitura_transformacao(df,regras):
  list_dfs = criaDFs(df)
  dict_return = {}
  for dfs in list_dfs:
