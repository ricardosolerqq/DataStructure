...
pre_outputpath = '/mnt/ml-prd/ml-data/propensaodeal/bahamas/output'
pre_outputpath_dbfs = '/dbfs/mnt/ml-prd/ml-data/propensaodeal/bahamas/output'
...
output = output.withColumn('Chave', F.split("Chave", ':'))
output = output.withColumn('Document', F.lpad(F.col("Chave").getItem(0),11,'0'))
output = output.groupBy(F.col('Document')).agg(F.max(F.col('GH')), F.max(F.col('P_1')), F.avg(F.col('P_1')))
output = output.withColumn('Provider', F.lit('qq_bahamas_propensity_v1'))
...
for arq in dbutils.fs.ls(os.path.join(pre_outputpath, 'temp_output.csv')):
  if arq.name.split('.')[-1] == 'csv':
    print (arq.path)
    dbutils.fs.cp(arq.path, os.path.join(pre_outputpath+'/bahamas_model_to_production_'+str(createdAt)+'.csv'))
...
output_portocred = pre_outputpath + '/bahamas_model_to_production_' + str(createdAt) + '.csv'
output_santander = santander_output + '/bahamas_model_to_production_' + str(createdAt) + '.csv'

dbutils.fs.cp(os.path.join(output_portocred), os.path.join(output_santander))

# COMMAND ----------

pre_outputpath+'/bahamas_model_to_production_'+str(createdAt)+'.csv'