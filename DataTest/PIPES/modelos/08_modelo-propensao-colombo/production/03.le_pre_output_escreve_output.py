...
pre_outputpath = '/mnt/ml-prd/ml-data/propensaodeal/colombo/output'
pre_outputpath_dbfs = '/dbfs/mnt/ml-prd/ml-data/propensaodeal/colombo/output'
...
output = output.withColumn('Provider', F.lit('qq_colombo_propensity_v1'))
...
for arq in dbutils.fs.ls(os.path.join(pre_outputpath, 'temp_output.csv')):
  if arq.name.split('.')[-1] == 'csv':
    print (arq.path)
    dbutils.fs.cp(arq.path, os.path.join(pre_outputpath,date,'colombo_model_to_production_'+str(createdAt_datetime_str)+'.csv'))
...
dbutils.fs.cp(os.path.join(pre_outputpath,date,'colombo_model_to_production_'+str(createdAt_datetime_str)+'.csv'), os.path.join(santander_output+'/colombo_model_to_production_'+str(createdAt_datetime_str)+'.csv'))
santander_output+'/colombo_model_to_production_'+str(createdAt_datetime_str)+'.csv'
