...
ID=pd.DataFrame(list(col_person.aggregate(pipeline=query_Ids_ativos,allowDiskUse=True)))['_id'].to_list()
...
# DBTITLE 1,Selecionando a data atual
data_hj=(datetime.today()-timedelta(hours=3)).strftime(format='%Y-%m-%d')

# COMMAND ----------

# DBTITLE 1,Removendo os arquivos anteriores
files=dbutils.fs.ls(dir_save)
dir_to_remove=list(map(lambda a : a.path,files))
del files

if len(dir_to_remove)>0:
  for i in dir_to_remove:
    print('Removendo a pasta "'+str.split(i,'/')[-2]+'...')
    dbutils.fs.rm(i, True)
else:
  print("Sem arquivos anteriores para remover")

# COMMAND ----------

# DBTITLE 1,Salvando
Tab_features.write.mode('overwrite').option("header",True).option("delimiter",";").option("emptyValue",'').csv(dir_save+'/data_base_to_score_'+data_hj)
