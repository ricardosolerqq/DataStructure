...
prefix = "etl/will/processed"

caminho_base = '/mnt/qq-integrator/etl/will/processed'
caminho_base_dbfs = '/dbfs/mnt/qq-integrator/etl/will/processed'
caminho_trusted = '/mnt/ml-prd/ml-data/propensaodeal/will/trusted'
caminho_base_models = '/mnt/ml-prd/ml-data/base_models'

file_name = 'Base_Will_QQ_Model_'+date+'.csv'

# COMMAND ----------

dates = []
files_dates = get_creditor_etl_file_dates('will', as_date=True)
...
dfDebtsCre = dfDebts.filter(F.col('creditor') == 'will').orderBy(F.col('_id').asc())
dfValCredor =  dfDebtsCre.select('_id','contract')

##Criando DF de divida nos demais credores
dfDebtsOut = dfDebts.filter(F.col('creditor') != 'will').select('_id','creditor').orderBy(F.col('_id').asc())
...
dfdeals = spark.read.format('delta').load(os.path.join(caminho_base_models, 'Deals', ArqDate)).filter(F.col('creditor')=='will')
...
dfinstallments = spark.read.format('delta').load(os.path.join(caminho_base_models, 'Installments', ArqDate))\
                 ...
                 .filter((F.col('DT_PAGAMENTO')>='2020-01-01') & (F.col('creditor')=='will'))\
...
dfinteractions = INTER_DATA.filter(F.col('creditor') == 'will')\
